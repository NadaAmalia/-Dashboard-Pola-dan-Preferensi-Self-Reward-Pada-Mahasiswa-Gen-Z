...
# SETUP DASHBOARD
# =========================
st.set_page_config(page_title="🎁 Dashboard Pola dan Preferensi Self-Reward Mahasiswa Gen Z", layout="wide")
...
st.markdown(
    """
    <div style="background-color:#4A89AC;padding:15px;border-radius:10px;margin-bottom:20px">
        <h1 style="color:white;text-align:center;">🎁 DASHBOARD POLA DAN PREFERENSI SELF-REWARD MAHASISWA GEN Z</h1>
    </div>
    """,
...
mean_budget = df['Budget_Reward'].mean()
mean_freq = df['Freq_Reward'].mean()
mean_freq_in = df['Freq_InginReward'].mean()

# Ubah rata-rata skala penting ke persen (maks 100%)
...
col4.metric("⭐ Persentase Kepentingan Self-Reward Mahasiswa Gen Z", f"{persentase_penting:.2f}%")

# =========================
# BOX PLOT (Budget Reward Saja)
# =========================
...
# =========================
st.markdown("---")
st.caption("📌 Dashboard interaktif pola reward mahasiswa | Dibuat dengan ❤️ pakai Streamlit")
